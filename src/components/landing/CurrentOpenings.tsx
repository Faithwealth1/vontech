    ...
    <article className="min-w-60 overflow-hidden w-[410px] rounded-[30px] border-2 border-solid border-[#E0E0E0] hover:shadow-lg transition-shadow cursor-pointer">
      <div className="w-full">
        <img
          src={imageUrl}
          alt={`${title} position`}
          className="block w-full h-auto object-cover rounded-[30px_30px_0px_0px]"
        />
      </div>
  ...
  return (
    <section className="z-0 w-full text-center bg-[#FBFBFB] pt-[60px] pb-[120px] px-20 max-md:max-w-full max-md:pb-[100px] max-md:px-5">
      <div className="w-full max-md:max-w-full">
        ...
        <div className="flex w-full flex-col gap-10 text-2xl text-[#181818] font-semibold leading-[1.3] mt-[60px] max-md:max-w-full max-md:mt-10">
          {jobOpeningRows.map((row, rowIndex) => (
            <div
              key={rowIndex}
              className="flex w-full gap-[40px_0px] justify-between flex-wrap"
            >
              {row.map((job, index) => (
                <JobCard
                  key={`${rowIndex}-${index}`}
